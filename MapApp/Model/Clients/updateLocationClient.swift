...
class updateLocationClient: UIViewController {
    ...
    class func update(newLocation: String, newURL: String, newLatitude: Double, newLongitude: Double, completion: @escaping (Bool, Error?) -> Void ){
        ...
        let updateJSON = resultsResponse(firstName: PublicStruct.firstName, lastName: PublicStruct.lastName, longitude: newLongitude, latitude: newLatitude, mapString: newLocation, mediaURL: newURL, uniqueKey: PublicStruct.uniqueKey, objectId: currentObjectId, createdAt: nil, updatedAt: nil)
